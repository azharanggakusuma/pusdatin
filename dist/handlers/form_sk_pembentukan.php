...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($sk_pembentukan == 'Lainnya' && !empty($inputLainnya)) {
        $sk_pembentukan = $inputLainnya;
    }

    // Check if the record already exists for the same year
    $check_query = "SELECT id FROM tb_sk_pembentukan WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    $check_result = mysqli_query($conn, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        // If record exists for the same year, update the existing record
        $sql = "UPDATE tb_sk_pembentukan 
                SET sk_pembentukan = '$sk_pembentukan', tahun = '$tahun' 
                WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    } else {
        // If record doesn't exist for the same year, insert a new record
        $sql = "INSERT INTO tb_sk_pembentukan (sk_pembentukan, user_id, desa_id, tahun)
                VALUES ('$sk_pembentukan', '$user_id', '$desa_id', '$tahun')";
    }
