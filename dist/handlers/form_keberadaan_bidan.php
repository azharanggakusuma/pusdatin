...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check_query = "SELECT id FROM tb_keberadaan_bidan WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    $check_result = mysqli_query($conn, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        // If record exists for the same year, update the existing record
        $sql = "UPDATE tb_keberadaan_bidan 
                SET keberadaan_bidan = '$keberadaan_bidan', tahun = '$tahun' 
                WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    } else {
        // If record doesn't exist for the same year, insert a new record
        $sql = "INSERT INTO tb_keberadaan_bidan (keberadaan_bidan, user_id, desa_id, tahun)
                VALUES ('$keberadaan_bidan', '$user_id', '$desa_id', '$tahun')";
    }

    if (mysqli_query($conn, $sql)) {
        // Check if progress entry exists for the same year
        $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Keberadaan Bidan Desa yang menetap di Desa/Kelurahan' AND tahun = '$tahun'";
        ...
        if (mysqli_num_rows($result_progress) > 0) {
            ...
            $update_progress = "UPDATE user_progress 
                                ...
                                WHERE user_id = '$user_id' AND form_name = 'Keberadaan Bidan Desa yang menetap di Desa/Kelurahan' AND tahun = '$tahun'";
            ...
            $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                                VALUES ('$user_id', 'Keberadaan Bidan Desa yang menetap di Desa/Kelurahan', TRUE, '$desa_id', '$created_at', '$tahun')";
