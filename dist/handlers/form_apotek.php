...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($_POST as $key => $value) {
        if (preg_match('/^nama_apotek_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
            $data[$index]['nama_apotek'] = mysqli_real_escape_string($conn, $value);
        } elseif (preg_match('/^alamat_apotek_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
            $data[$index]['alamat_apotek'] = mysqli_real_escape_string($conn, $value);
    ...
    foreach ($data as $apotek) {
        $nama_apotek = $apotek['nama_apotek'] ?? '';
        $alamat_apotek = $apotek['alamat_apotek'] ?? '';
        $nama_kecamatan = $apotek['nama_kecamatan'] ?? '';
        $koordinat_lintang = $apotek['koordinat_lintang'] ?? '';
        $koordinat_bujur = $apotek['koordinat_bujur'] ?? '';

        if ($nama_apotek && $alamat_apotek && $nama_kecamatan && $koordinat_lintang && $koordinat_bujur) {
            // Check if the record already exists
            $check_query = "SELECT id FROM tb_apotek WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun' AND nama_apotek = '$nama_apotek'";
            $check_result = mysqli_query($conn, $check_query);

            if (mysqli_num_rows($check_result) > 0) {
                // If record exists, update it
                $update_sql = "UPDATE tb_apotek 
                               SET alamat_apotek = '$alamat_apotek', 
                ...
                $insert_sql = "INSERT INTO tb_apotek (nama_apotek, alamat_apotek, nama_kecamatan, koordinat_lintang, koordinat_bujur, tahun, user_id, desa_id)
                               VALUES ('$nama_apotek', '$alamat_apotek', '$nama_kecamatan', '$koordinat_lintang', '$koordinat_bujur', '$tahun', '$user_id', '$desa_id')";
    ...
    $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Apotek' AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($result_progress) > 0) {
        ...
        $update_progress = "UPDATE user_progress 
                            ...
                            WHERE user_id = '$user_id' AND form_name = 'Apotek' AND tahun = '$tahun'";
        ...
        $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                            VALUES ('$user_id', 'Apotek', TRUE, '$desa_id', '$created_at', '$tahun')";
