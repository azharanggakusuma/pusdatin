...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($_POST as $key => $value) {
        if (preg_match('/^nama_polindes_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
            $data[$index]['nama_polindes'] = mysqli_real_escape_string($conn, $value);
        } elseif (preg_match('/^alamat_polindes_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
            $data[$index]['alamat_polindes'] = mysqli_real_escape_string($conn, $value);
    ...
    foreach ($data as $polindes) {
        $nama_polindes = $polindes['nama_polindes'] ?? '';
        $alamat_polindes = $polindes['alamat_polindes'] ?? '';
        $nama_kecamatan = $polindes['nama_kecamatan'] ?? '';
        $koordinat_lintang = $polindes['koordinat_lintang'] ?? '';
        $koordinat_bujur = $polindes['koordinat_bujur'] ?? '';

        if ($nama_polindes && $alamat_polindes && $nama_kecamatan && $koordinat_lintang && $koordinat_bujur) {
            // Check if the record already exists
            $check_query = "SELECT id FROM tb_polindes WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun' AND nama_polindes = '$nama_polindes'";
            $check_result = mysqli_query($conn, $check_query);

            if (mysqli_num_rows($check_result) > 0) {
                // If record exists, update it
                $update_sql = "UPDATE tb_polindes 
                               SET alamat_polindes = '$alamat_polindes', 
                               ...
                               AND nama_polindes = '$nama_polindes'";
                ...
                $insert_sql = "INSERT INTO tb_polindes (nama_polindes, alamat_polindes, nama_kecamatan, koordinat_lintang, koordinat_bujur, tahun, user_id, desa_id)
                               VALUES ('$nama_polindes', '$alamat_polindes', '$nama_kecamatan', '$koordinat_lintang', '$koordinat_bujur', '$tahun', '$user_id', '$desa_id')";
    ...
    $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Polindes' AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($result_progress) > 0) {
        ...
        $update_progress = "UPDATE user_progress 
                            ...
                            WHERE user_id = '$user_id' AND form_name = 'Polindes' AND tahun = '$tahun'";
        ...
        $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                            VALUES ('$user_id', 'Polindes', TRUE, '$desa_id', '$created_at', '$tahun')";
