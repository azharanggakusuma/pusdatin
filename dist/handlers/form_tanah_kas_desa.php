...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ============================
    $tanah_bengkok = (float)mysqli_real_escape_string($conn, $_POST['tanah_bengkok']);
    $tanah_titi_sara = (float)mysqli_real_escape_string($conn, $_POST['tanah_titi_sara']);
    $kebun_desa = (float)mysqli_real_escape_string($conn, $_POST['kebun_desa']);
    $sawah_desa = (float)mysqli_real_escape_string($conn, $_POST['sawah_desa']);

    // ============================
    // Cek apakah data untuk tahun yang sama sudah ada di database
    // ============================
    $check_query = "SELECT id FROM tb_tanah_kas_desa WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    $check_result = mysqli_query($conn, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        // Jika data sudah ada, lakukan UPDATE
        $sql = "UPDATE tb_tanah_kas_desa SET 
                tanah_bengkok = '$tanah_bengkok', 
                tanah_titi_sara = '$tanah_titi_sara', 
                kebun_desa = '$kebun_desa', 
                sawah_desa = '$sawah_desa' 
                WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    } else {
        // Jika data belum ada, lakukan INSERT
        $sql = "INSERT INTO tb_tanah_kas_desa (tanah_bengkok, tanah_titi_sara, kebun_desa, sawah_desa, tahun, user_id, desa_id) 
                VALUES ('$tanah_bengkok', '$tanah_titi_sara', '$kebun_desa', '$sawah_desa', '$tahun', '$user_id', '$desa_id')";
    }
