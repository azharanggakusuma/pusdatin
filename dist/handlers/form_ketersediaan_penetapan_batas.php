...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $no_surat_batas_desa = ($penetapan_batas_desa === 'SUDAH ADA') ? mysqli_real_escape_string($conn, $_POST['no_surat_batas_desa']) : 'BELUM ADA';

    $ketersediaan_peta_desa = mysqli_real_escape_string($conn, $_POST['ketersediaan_peta_desa']);
    $no_surat_peta_desa = ($ketersediaan_peta_desa === 'ADA') ? mysqli_real_escape_string($conn, $_POST['no_surat_peta_desa']) : 'TIDAK ADA';
    ...
    $sql = "INSERT INTO tb_ketersediaan_penetapan_batas (penetapan_batas_desa, no_surat_batas_desa, ketersediaan_peta_desa, no_surat_peta_desa, user_id, desa_id) 
            VALUES ('$penetapan_batas_desa', '$no_surat_batas_desa', '$ketersediaan_peta_desa', '$no_surat_peta_desa', '$user_id', '$desa_id')";

    if (mysqli_query($conn, $sql)) {
        // Tambahkan atau perbarui progres pengguna
        $query_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id) 
                           VALUES ('$user_id', 'Ketersediaan Penetapan Batas dan Peta Desa', TRUE, '$desa_id')
