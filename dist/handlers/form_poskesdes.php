...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($_POST as $key => $value) {
        if (preg_match('/^nama_poskesdes_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
            $data[$index]['nama_poskesdes'] = mysqli_real_escape_string($conn, $value);
        } elseif (preg_match('/^alamat_poskesdes_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
            $data[$index]['alamat_poskesdes'] = mysqli_real_escape_string($conn, $value);
    ...
    foreach ($data as $poskesdes) {
        $nama_poskesdes = $poskesdes['nama_poskesdes'] ?? '';
        $alamat_poskesdes = $poskesdes['alamat_poskesdes'] ?? '';
        $nama_kecamatan = $poskesdes['nama_kecamatan'] ?? '';
        $koordinat_lintang = $poskesdes['koordinat_lintang'] ?? '';
        $koordinat_bujur = $poskesdes['koordinat_bujur'] ?? '';

        if ($nama_poskesdes && $alamat_poskesdes && $nama_kecamatan && $koordinat_lintang && $koordinat_bujur) {
            // Check if the record already exists
            $check_query = "SELECT id FROM tb_poskesdes WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun' AND nama_poskesdes = '$nama_poskesdes'";
            $check_result = mysqli_query($conn, $check_query);

            if (mysqli_num_rows($check_result) > 0) {
                // If record exists, update it
                $update_sql = "UPDATE tb_poskesdes 
                               SET alamat_poskesdes = '$alamat_poskesdes', 
                               ...
                               AND nama_poskesdes = '$nama_poskesdes'";
                ...
                $insert_sql = "INSERT INTO tb_poskesdes (nama_poskesdes, alamat_poskesdes, nama_kecamatan, koordinat_lintang, koordinat_bujur, tahun, user_id, desa_id)
                               VALUES ('$nama_poskesdes', '$alamat_poskesdes', '$nama_kecamatan', '$koordinat_lintang', '$koordinat_bujur', '$tahun', '$user_id', '$desa_id')";
    ...
    $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Poskesdes' AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($result_progress) > 0) {
        ...
        $update_progress = "UPDATE user_progress 
                            ...
                            WHERE user_id = '$user_id' AND form_name = 'Poskesdes' AND tahun = '$tahun'";
        ...
        $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                            VALUES ('$user_id', 'Poskesdes', TRUE, '$desa_id', '$created_at', '$tahun')";
