...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($data as $posyandu) {
        ...
        $nama_kecamatan = $posyandu['nama_kecamatan'] ?? '';
        $koordinat_lintang = $posyandu['koordinat_lintang'] ?? '';
        $koordinat_bujur = $posyandu['koordinat_bujur'] ?? '';

        if ($nama_posyandu && $alamat_posyandu && $nama_kecamatan && $koordinat_lintang && $koordinat_bujur) {
            // Check if the record already exists
            $check_query = "SELECT id FROM tb_daftar_posyandu WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun' AND nama_posyandu = '$nama_posyandu'";
            $check_result = mysqli_query($conn, $check_query);

            if (mysqli_num_rows($check_result) > 0) {
                // If record exists, update it
                $update_sql = "UPDATE tb_daftar_posyandu 
                               SET alamat_posyandu = '$alamat_posyandu', 
                               ...
                               AND nama_posyandu = '$nama_posyandu'";
                ...
                $insert_sql = "INSERT INTO tb_daftar_posyandu (nama_posyandu, alamat_posyandu, nama_kecamatan, koordinat_lintang, koordinat_bujur, tahun, user_id, desa_id)
                               VALUES ('$nama_posyandu', '$alamat_posyandu', '$nama_kecamatan', '$koordinat_lintang', '$koordinat_bujur', '$tahun', '$user_id', '$desa_id')";
    ...
    $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Daftar Posyandu' AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($result_progress) > 0) {
        ...
        $update_progress = "UPDATE user_progress 
                            ...
                            WHERE user_id = '$user_id' AND form_name = 'Daftar Posyandu' AND tahun = '$tahun'";
        ...
        $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                            VALUES ('$user_id', 'Daftar Posyandu', TRUE, '$desa_id', '$created_at', '$tahun')";
