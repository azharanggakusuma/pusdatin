...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ============================
    $jumlah_anggota_linmas = (int)mysqli_real_escape_string($conn, $_POST['jumlah_anggota_linmas']);

    // ============================
    // Cek apakah data untuk tahun yang sama sudah ada di database
    // ============================
    $check_query = "SELECT id FROM tb_linmas_poskamling WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    $check_result = mysqli_query($conn, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        // Jika data sudah ada, lakukan UPDATE
        $sql = "UPDATE tb_linmas_poskamling SET 
            jumlah_anggota_linmas = '$jumlah_anggota_linmas' 
            WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    } else {
        // Jika data belum ada, lakukan INSERT
        $sql = "INSERT INTO tb_linmas_poskamling (jumlah_anggota_linmas, tahun, user_id, desa_id) 
                VALUES ('$jumlah_anggota_linmas', '$tahun', '$user_id', '$desa_id')";
    }
