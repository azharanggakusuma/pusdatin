...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $keberadaan_sungai = mysqli_real_escape_string($conn, $_POST['keberadaan_sungai']);
  $nama_sungai_1 = mysqli_real_escape_string($conn, $_POST['nama_sungai_1'] ?? null);
  $nama_sungai_2 = mysqli_real_escape_string($conn, $_POST['nama_sungai_2'] ?? null);
  $nama_sungai_3 = mysqli_real_escape_string($conn, $_POST['nama_sungai_3'] ?? null);
  $nama_sungai_4 = mysqli_real_escape_string($conn, $_POST['nama_sungai_4'] ?? null);

  // Check if the record already exists for the same year
  $check_query = "SELECT id FROM tb_keberadaan_sungai WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
  $check_result = mysqli_query($conn, $check_query);

  if (mysqli_num_rows($check_result) > 0) {
    // If record exists for the same year, update the existing record
    $sql = "UPDATE tb_keberadaan_sungai 
                SET keberadaan_sungai = '$keberadaan_sungai', 
                    nama_sungai_1 = '$nama_sungai_1', 
                    nama_sungai_2 = '$nama_sungai_2', 
                    nama_sungai_3 = '$nama_sungai_3', 
                    nama_sungai_4 = '$nama_sungai_4', 
                    tahun = '$tahun' 
                WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
  } else {
    // If record doesn't exist for the same year, insert a new record
    $sql = "INSERT INTO tb_keberadaan_sungai (keberadaan_sungai, nama_sungai_1, nama_sungai_2, nama_sungai_3, nama_sungai_4, user_id, desa_id, tahun)
                VALUES ('$keberadaan_sungai', '$nama_sungai_1', '$nama_sungai_2', '$nama_sungai_3', '$nama_sungai_4', '$user_id', '$desa_id', '$tahun')";
  }

  if (mysqli_query($conn, $sql)) {
    // Check if progress entry exists for the same year
    $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Keberadaan Sungai' AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($result_progress) > 0) {
      ...
      $update_progress = "UPDATE user_progress 
                                ...
                                WHERE user_id = '$user_id' AND form_name = 'Keberadaan Sungai' AND tahun = '$tahun'";
      ...
      $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                                VALUES ('$user_id', 'Keberadaan Sungai', TRUE, '$desa_id', '$created_at', '$tahun')";
