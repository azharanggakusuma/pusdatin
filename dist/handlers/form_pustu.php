...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($_POST as $key => $value) {
        if (preg_match('/^nama_pustu_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
            $data[$index]['nama_pustu'] = mysqli_real_escape_string($conn, $value);
        } elseif (preg_match('/^alamat_pustu_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
            $data[$index]['alamat_pustu'] = mysqli_real_escape_string($conn, $value);
        } elseif (preg_match('/^nama_kecamatan_pustu_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
    ...
    foreach ($data as $pustu) {
        $nama_pustu = $pustu['nama_pustu'] ?? '';
        $alamat_pustu = $pustu['alamat_pustu'] ?? '';
        $nama_kecamatan = $pustu['nama_kecamatan'] ?? '';
        $koordinat_lintang = $pustu['koordinat_lintang'] ?? '';
        $koordinat_bujur = $pustu['koordinat_bujur'] ?? '';

        if ($nama_pustu && $alamat_pustu && $nama_kecamatan && $koordinat_lintang && $koordinat_bujur) {
            // Check if the record already exists
            $check_query = "SELECT id FROM tb_pustu WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun' AND nama_pustu = '$nama_pustu'";
            $check_result = mysqli_query($conn, $check_query);

            if (mysqli_num_rows($check_result) > 0) {
                // If record exists, update it
                $update_sql = "UPDATE tb_pustu 
                               SET alamat_pustu = '$alamat_pustu', 
                ...
                $insert_sql = "INSERT INTO tb_pustu (nama_pustu, alamat_pustu, nama_kecamatan, koordinat_lintang, koordinat_bujur, tahun, user_id, desa_id)
                               VALUES ('$nama_pustu', '$alamat_pustu', '$nama_kecamatan', '$koordinat_lintang', '$koordinat_bujur', '$tahun', '$user_id', '$desa_id')";
    ...
    $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Pustu' AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($result_progress) > 0) {
        ...
        $update_progress = "UPDATE user_progress 
                            ...
                            WHERE user_id = '$user_id' AND form_name = 'Pustu' AND tahun = '$tahun'";
        ...
        $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                            VALUES ('$user_id', 'Pustu', TRUE, '$desa_id', '$created_at', '$tahun')";
