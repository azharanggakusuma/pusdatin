...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $topografi_terluas_wilayah_desa = mysqli_real_escape_string($conn, $_POST['topografi_terluas_wilayah_desa'] ?? '');

    // ============================
    // Bagian VALIDASI DASAR
    // ============================

    // 1) Pastikan user memilih topografi (tidak boleh kosong)
    $allowed_topografi = ['LERENG/PUNCAK', 'LEMBAH', 'DATARAN', 'PESISIR PANTAI'];
    if (empty($topografi_terluas_wilayah_desa) || !in_array($topografi_terluas_wilayah_desa, $allowed_topografi)) {
        $message = "Anda harus memilih salah satu Topografi Terluas Wilayah Desa.";
    ...
    $check_query = "SELECT id 
                    FROM tb_topografi_terluas_wilayah_desa 
                    WHERE user_id = '$user_id' 
                      AND desa_id = '$desa_id' 
                      AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $sql = "UPDATE tb_topografi_terluas_wilayah_desa
                SET topografi_terluas_wilayah_desa = '$topografi_terluas_wilayah_desa',
                    tahun = '$tahun'
                WHERE user_id = '$user_id'
                  AND desa_id = '$desa_id'
                  AND tahun = '$tahun'";
        ...
        $sql = "INSERT INTO tb_topografi_terluas_wilayah_desa (
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        $query_progress = "SELECT id
                           FROM user_progress
                           WHERE user_id = '$user_id'
                             AND form_name = 'Topografi Terluas Wilayah Desa'
                             AND tahun = '$tahun'";
