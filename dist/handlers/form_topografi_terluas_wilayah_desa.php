...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $topografi_terluas = mysqli_real_escape_string($conn, $_POST['topografi_terluas_wilayah_desa']);

    if (!empty($topografi_terluas)) {
        // Perbarui query untuk memasukkan user_id dan desa_id
        $sql = "INSERT INTO tb_topografi_terluas_wilayah_desa (topografi_terluas_wilayah_desa, user_id, desa_id) 
                VALUES ('$topografi_terluas', '$user_id', '$desa_id')";

        if (mysqli_query($conn, $sql)) {
            // Tambahkan atau perbarui progres pengguna
            $query_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id) 
                               VALUES ('$user_id', 'Topografi Terluas Wilayah Desa', TRUE, '$desa_id')
