...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($_POST as $key => $value) {
        if (preg_match('/^nama_puskesmas_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
            $data[$index]['nama_puskesmas'] = mysqli_real_escape_string($conn, $value);
        } elseif (preg_match('/^alamat_puskesmas_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
            $data[$index]['alamat_puskesmas'] = mysqli_real_escape_string($conn, $value);
        } elseif (preg_match('/^nama_kecamatan_puskesmas_(\d+)$/', $key, $matches)) {
            $index = $matches[1];
    ...
    foreach ($data as $puskesmas) {
        ...
        $nama_kecamatan = $puskesmas['nama_kecamatan'] ?? '';
        $koordinat_lintang = $puskesmas['koordinat_lintang'] ?? '';
        $koordinat_bujur = $puskesmas['koordinat_bujur'] ?? '';

        if ($nama_puskesmas && $alamat_puskesmas && $nama_kecamatan && $koordinat_lintang && $koordinat_bujur) {
            // Check if the record already exists
            $check_query = "SELECT id FROM tb_puskesmas WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun' AND nama_puskesmas = '$nama_puskesmas'";
            $check_result = mysqli_query($conn, $check_query);

            if (mysqli_num_rows($check_result) > 0) {
                // If record exists, update it
                $update_sql = "UPDATE tb_puskesmas 
                               SET alamat_puskesmas = '$alamat_puskesmas', 
                               ...
                               AND nama_puskesmas = '$nama_puskesmas'";
                ...
                $insert_sql = "INSERT INTO tb_puskesmas (nama_puskesmas, alamat_puskesmas, nama_kecamatan, koordinat_lintang, koordinat_bujur, tahun, user_id, desa_id)
                               VALUES ('$nama_puskesmas', '$alamat_puskesmas', '$nama_kecamatan', '$koordinat_lintang', '$koordinat_bujur', '$tahun', '$user_id', '$desa_id')";
    ...
    $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Puskesmas' AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($result_progress) > 0) {
        ...
        $update_progress = "UPDATE user_progress 
                            ...
                            WHERE user_id = '$user_id' AND form_name = 'Puskesmas' AND tahun = '$tahun'";
        ...
        $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                            VALUES ('$user_id', 'Puskesmas', TRUE, '$desa_id', '$created_at', '$tahun')";
