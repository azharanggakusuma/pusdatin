...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $dasar_hukum = mysqli_real_escape_string($conn, $_POST['dasar_hukum']);
    $no_peraturan = mysqli_real_escape_string($conn, $_POST['no_peraturan']);

    // Jika dasar hukum adalah "TIDAK ADA", set no_peraturan menjadi "TIDAK ADA"
    if ($dasar_hukum === 'TIDAK ADA') {
        $no_peraturan = 'TIDAK ADA';
    }
    ...
    $sql = "INSERT INTO tb_dasar_hukum_pemerintahan_desa (dasar_hukum, no_peraturan, user_id, desa_id) 
            VALUES ('$dasar_hukum', '$no_peraturan', '$user_id', '$desa_id')";

    if (mysqli_query($conn, $sql)) {
        // Tambahkan atau perbarui progres pengguna
        $query_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id) 
                           VALUES ('$user_id', 'Dasar hukum pembentukan Pemerintah Desa / Kelurahan', TRUE, '$desa_id')
