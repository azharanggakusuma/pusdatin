...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($data as $praktek) {
        $nama_praktek_bidan = $praktek['nama_praktek_bidan'] ?? '';
        $alamat_praktek_bidan = $praktek['alamat_praktek_bidan'] ?? '';
        $nama_kecamatan = $praktek['nama_kecamatan'] ?? '';
        $koordinat_lintang = $praktek['koordinat_lintang'] ?? '';
        $koordinat_bujur = $praktek['koordinat_bujur'] ?? '';

        if ($nama_praktek_bidan && $alamat_praktek_bidan && $nama_kecamatan && $koordinat_lintang && $koordinat_bujur) {
            // Check if the record already exists
            $check_query = "SELECT id FROM tb_praktek_bidan WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun' AND nama_praktek_bidan = '$nama_praktek_bidan'";
            $check_result = mysqli_query($conn, $check_query);

            if (mysqli_num_rows($check_result) > 0) {
                // If record exists, update it
                $update_sql = "UPDATE tb_praktek_bidan 
                               SET alamat_praktek_bidan = '$alamat_praktek_bidan', 
                               ...
                               AND nama_praktek_bidan = '$nama_praktek_bidan'";
                ...
                $insert_sql = "INSERT INTO tb_praktek_bidan (nama_praktek_bidan, alamat_praktek_bidan, nama_kecamatan, koordinat_lintang, koordinat_bujur, tahun, user_id, desa_id)
                               VALUES ('$nama_praktek_bidan', '$alamat_praktek_bidan', '$nama_kecamatan', '$koordinat_lintang', '$koordinat_bujur', '$tahun', '$user_id', '$desa_id')";
    ...
    $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Praktek Bidan' AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($result_progress) > 0) {
        ...
        $update_progress = "UPDATE user_progress 
                            ...
                            WHERE user_id = '$user_id' AND form_name = 'Praktek Bidan' AND tahun = '$tahun'";
        ...
        $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                            VALUES ('$user_id', 'Praktek Bidan', TRUE, '$desa_id', '$created_at', '$tahun')";
