...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $status_badan_hukum = mysqli_real_escape_string($conn, $_POST['status_badan_hukum']);

    // ============================
    // Cek apakah data untuk tahun yang sama sudah ada di database
    // ============================
    $check_query = "SELECT id FROM tb_bumdes WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    $check_result = mysqli_query($conn, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        // Jika data sudah ada, lakukan UPDATE
        $sql = "UPDATE tb_bumdes SET 
        ...
        $sql = "INSERT INTO tb_bumdes (
                    status_keaktifan, status_badan_hukum, tahun, user_id, desa_id
                ) VALUES (
                    '$status_keaktifan', '$status_badan_hukum', '$tahun', '$user_id', '$desa_id'
                )";
    }
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        // ============================
        $form_name = 'Badan Usaha Milik Desa (BUMDes)';
