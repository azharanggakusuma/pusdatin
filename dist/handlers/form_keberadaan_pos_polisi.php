...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ============================
    $keberadaan_pos_polisi = mysqli_real_escape_string($conn, $_POST['keberadaan_pos_polisi']);

    // ============================
    // Cek apakah data untuk tahun yang sama sudah ada di database
    // ============================
    $check_query = "SELECT id FROM tb_keberadaan_pos_polisi WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    $check_result = mysqli_query($conn, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        // Jika data sudah ada, lakukan UPDATE
        $sql = "UPDATE tb_keberadaan_pos_polisi SET 
                keberadaan_pos_polisi = '$keberadaan_pos_polisi'
                WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    } else {
        // Jika data belum ada, lakukan INSERT
        $sql = "INSERT INTO tb_keberadaan_pos_polisi (keberadaan_pos_polisi, tahun, user_id, desa_id) 
                VALUES ('$keberadaan_pos_polisi', '$tahun', '$user_id', '$desa_id')";
    }
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        // ============================
        $form_name = 'Keberadaan Pos Polisi';
