...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($data as $rumahSakit) {
        $nama_rumah_sakit = $rumahSakit['nama_rumah_sakit'] ?? '';
        $alamat_rumah_sakit = $rumahSakit['alamat_rumah_sakit'] ?? '';
        $nama_kecamatan = $rumahSakit['nama_kecamatan'] ?? '';
        $koordinat_lintang = $rumahSakit['koordinat_lintang'] ?? '';
        $koordinat_bujur = $rumahSakit['koordinat_bujur'] ?? '';

        if ($nama_rumah_sakit && $alamat_rumah_sakit && $nama_kecamatan && $koordinat_lintang && $koordinat_bujur) {
            // Check if the record already exists
            $check_query = "SELECT id FROM tb_rumah_sakit WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun' AND nama_rumah_sakit = '$nama_rumah_sakit'";
            $check_result = mysqli_query($conn, $check_query);

            if (mysqli_num_rows($check_result) > 0) {
                // If record exists, update it
                $update_sql = "UPDATE tb_rumah_sakit 
                               SET alamat_rumah_sakit = '$alamat_rumah_sakit', 
                               ...
                               AND nama_rumah_sakit = '$nama_rumah_sakit'";
                ...
                $insert_sql = "INSERT INTO tb_rumah_sakit (nama_rumah_sakit, alamat_rumah_sakit, nama_kecamatan, koordinat_lintang, koordinat_bujur, tahun, user_id, desa_id)
                               VALUES ('$nama_rumah_sakit', '$alamat_rumah_sakit', '$nama_kecamatan', '$koordinat_lintang', '$koordinat_bujur', '$tahun', '$user_id', '$desa_id')";
    ...
    $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Rumah Sakit' AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($result_progress) > 0) {
        ...
        $update_progress = "UPDATE user_progress 
                            ...
                            WHERE user_id = '$user_id' AND form_name = 'Rumah Sakit' AND tahun = '$tahun'";
        ...
        $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                            VALUES ('$user_id', 'Rumah Sakit', TRUE, '$desa_id', '$created_at', '$tahun')";
