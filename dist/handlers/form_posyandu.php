...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $jumlah_posyandu = intval($_POST['jumlah_posyandu'] ?? 0);

    // Check if the record already exists for the same year
    $check_query = "SELECT id FROM tb_posyandu WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    $check_result = mysqli_query($conn, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        // If record exists for the same year, update the existing record
        $sql = "UPDATE tb_posyandu 
                SET jumlah_posyandu = '$jumlah_posyandu', 
                    tahun = '$tahun' 
                WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    } else {
        // If record doesn't exist for the same year, insert a new record
        $sql = "INSERT INTO tb_posyandu 
                (jumlah_posyandu, user_id, desa_id, tahun)
                VALUES ('$jumlah_posyandu', '$user_id', '$desa_id', '$tahun')";
    }
