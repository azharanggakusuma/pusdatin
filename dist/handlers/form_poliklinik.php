...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($data as $poliklinik) {
        ...
        $alamat_poliklinik = $poliklinik['alamat_poliklinik'] ?? '';
        $nama_kecamatan = $poliklinik['nama_kecamatan'] ?? '';
        $koordinat_lintang = $poliklinik['koordinat_lintang'] ?? '';
        $koordinat_bujur = $poliklinik['koordinat_bujur'] ?? '';

        if ($nama_poliklinik && $alamat_poliklinik && $nama_kecamatan && $koordinat_lintang && $koordinat_bujur) {
            // Check if the record already exists
            $check_query = "SELECT id FROM tb_poliklinik WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun' AND nama_poliklinik = '$nama_poliklinik'";
            $check_result = mysqli_query($conn, $check_query);

            if (mysqli_num_rows($check_result) > 0) {
                // If record exists, update it
                $update_sql = "UPDATE tb_poliklinik 
                               SET alamat_poliklinik = '$alamat_poliklinik', 
                               ...
                               AND nama_poliklinik = '$nama_poliklinik'";
                ...
                $insert_sql = "INSERT INTO tb_poliklinik (nama_poliklinik, alamat_poliklinik, nama_kecamatan, koordinat_lintang, koordinat_bujur, tahun, user_id, desa_id)
                               VALUES ('$nama_poliklinik', '$alamat_poliklinik', '$nama_kecamatan', '$koordinat_lintang', '$koordinat_bujur', '$tahun', '$user_id', '$desa_id')";
    ...
    $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Poliklinik' AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($result_progress) > 0) {
        ...
        $update_progress = "UPDATE user_progress 
                            ...
                            WHERE user_id = '$user_id' AND form_name = 'Poliklinik' AND tahun = '$tahun'";
        ...
        $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                            VALUES ('$user_id', 'Poliklinik', TRUE, '$desa_id', '$created_at', '$tahun')";
