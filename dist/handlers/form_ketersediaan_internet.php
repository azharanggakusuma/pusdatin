...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $kondisi_komputer = mysqli_real_escape_string($conn, $_POST['kondisi_komputer']);
    $fasilitas_internet = mysqli_real_escape_string($conn, $_POST['fasilitas_internet']);

    // Check if the record already exists for the same year
    $check_query = "SELECT id FROM tb_ketersediaan_internet WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    $check_result = mysqli_query($conn, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        // Update existing record
        $sql = "UPDATE tb_ketersediaan_internet 
                SET kondisi_komputer = '$kondisi_komputer', fasilitas_internet = '$fasilitas_internet', tahun = '$tahun' 
                WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    } else {
        // Insert new record
        $sql = "INSERT INTO tb_ketersediaan_internet (kondisi_komputer, fasilitas_internet, user_id, desa_id, tahun)
                VALUES ('$kondisi_komputer', '$fasilitas_internet', '$user_id', '$desa_id', '$tahun')";
    }

    if (mysqli_query($conn, $sql)) {
        // Update progress
        $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Ketersediaan Internet dan Komputer' AND tahun = '$tahun'";
        ...
        if (mysqli_num_rows($result_progress) > 0) {
            $update_progress = "UPDATE user_progress 
                                ...
                                WHERE user_id = '$user_id' AND form_name = 'Ketersediaan Internet dan Komputer' AND tahun = '$tahun'";
            ...
            $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                                VALUES ('$user_id', 'Ketersediaan Internet dan Komputer', TRUE, '$desa_id', '$created_at', '$tahun')";
