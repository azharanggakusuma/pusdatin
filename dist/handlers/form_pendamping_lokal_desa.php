...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ============================
    $keberadaan_pendamping_lokal_desa = mysqli_real_escape_string($conn, $_POST['keberadaan_pendamping_lokal_desa']);

    // ============================
    // Cek apakah data untuk tahun yang sama sudah ada di database
    // ============================
    $check_query = "SELECT id FROM tb_pendamping_lokal_desa WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    $check_result = mysqli_query($conn, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        // Jika data sudah ada, lakukan UPDATE
        $sql = "UPDATE tb_pendamping_lokal_desa SET 
                keberadaan_pendamping_lokal_desa = '$keberadaan_pendamping_lokal_desa'
                WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
    } else {
        // Jika data belum ada, lakukan INSERT
        $sql = "INSERT INTO tb_pendamping_lokal_desa (
                    keberadaan_pendamping_lokal_desa, tahun, user_id, desa_id
                ) VALUES (
                    '$keberadaan_pendamping_lokal_desa', '$tahun', '$user_id', '$desa_id'
                )";
    }
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        // ============================
        $form_name = 'Keberadaan Pendamping Lokal Desa';
