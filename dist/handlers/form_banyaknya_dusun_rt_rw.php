...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validasi input
    if (empty($jumlah_dusun) || empty($jumlah_rw) || empty($jumlah_rt)) {
    ...
    $sql = "INSERT INTO tb_dusun_rw_rt (jumlah_dusun, jumlah_rw, jumlah_rt, user_id, desa_id) 
            VALUES ('$jumlah_dusun', '$jumlah_rw', '$jumlah_rt', '$user_id', '$desa_id')";

    if (mysqli_query($conn, $sql)) {
        // Tambahkan atau perbarui progres pengguna
        $query_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id) 
                           VALUES ('$user_id', 'Banyaknya Dusun, Rukun Tetangga dan Rukun Warga', TRUE, '$desa_id')
