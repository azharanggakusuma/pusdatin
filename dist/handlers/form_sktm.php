...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $jumlah_sktm = mysqli_real_escape_string($conn, $_POST['jumlah_sktm']);

  // Check if the record already exists for the same year
  $check_query = "SELECT id FROM tb_sktm WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
  $check_result = mysqli_query($conn, $check_query);

  if (mysqli_num_rows($check_result) > 0) {
    // If record exists for the same year, update the existing record
    $sql = "UPDATE tb_sktm 
                SET jumlah_sktm = '$jumlah_sktm', 
                    tahun = '$tahun' 
                WHERE user_id = '$user_id' AND desa_id = '$desa_id' AND tahun = '$tahun'";
  } else {
    // If record doesn't exist for the same year, insert a new record
    $sql = "INSERT INTO tb_sktm (jumlah_sktm, user_id, desa_id, tahun)
                VALUES ('$jumlah_sktm', '$user_id', '$desa_id', '$tahun')";
  }

  if (mysqli_query($conn, $sql)) {
    // Check if progress entry exists for the same year
    $query_progress = "SELECT id FROM user_progress WHERE user_id = '$user_id' AND form_name = 'Jumlah SKTM' AND tahun = '$tahun'";
    ...
    if (mysqli_num_rows($result_progress) > 0) {
      ...
      $update_progress = "UPDATE user_progress 
                                ...
                                WHERE user_id = '$user_id' AND form_name = 'Jumlah SKTM' AND tahun = '$tahun'";
      ...
      $insert_progress = "INSERT INTO user_progress (user_id, form_name, is_locked, desa_id, created_at, tahun) 
                                VALUES ('$user_id', 'Jumlah SKTM', TRUE, '$desa_id', '$created_at', '$tahun')";
